...
/**@brief 用于旋转矩阵、欧拉角和四元数之间的相互转换(四元数依赖Quaternion). The Matrix3x3 class implements a 3x3 rotation matrix, to perform linear algebra in combination with Quaternion, Transform and Vector3.
* Make sure to only include a pure orthogonal matrix without scaling. */
    ...
    public:
        ...
        * @param s Scaling vector The elements of the vector will scale each column */
        ...
        tfScalar tdotx(const Vector3 &v) const {
            return m_el[0].x() * v.x() + m_el[1].x() * v.y() + m_el[2].x() * v.z();
        }

        tfScalar tdoty(const Vector3 &v) const {
            return m_el[0].y() * v.x() + m_el[1].y() * v.y() + m_el[2].y() * v.z();
        }

        tfScalar tdotz(const Vector3 &v) const {
